...
def test_response_non_streaming_file_search(
    ...
    assert_text_contains(response.output[0].results[0].text, case.expected)
...
def test_response_sequential_file_search(
    ...
    assert response.output[0].results
    assert "128" in response.output_text or "experts" in normalize_text(response.output_text)
...
def test_response_non_streaming_mcp_tool(responses_client, text_model_id, case, caplog):
    ...
    with make_mcp_server() as mcp_server_info:
        ...
        assert_text_contains(text_content, "boiling point")
...
def test_response_sequential_mcp_tool(responses_client, text_model_id, case):
    ...
    with make_mcp_server() as mcp_server_info:
        ...
        message = response.output[-1]
        text_content = message.content[0].text
        assert normalize_text("boiling point") in normalize_text(text_content) or "-100" in text_content, (
            f"Expected 'boiling point' or tool result '-100' in response: {text_content}"
        )
...
def test_response_function_call_ordering_2(responses_client, text_model_id):
    ...
    assert len(response.output) == 1
    assert_text_contains(response.output_text, "Los Angeles")
...
def test_response_streaming_multi_turn_tool_execution(responses_client, text_model_id, case):
    ...
    with make_mcp_server(tools=dependency_tools()) as mcp_server_info:
        ...
        if hasattr(final_chunk, "response"):
            ...
            assert_text_contains(final_response.output_text, case.expected)
